...
class Global_Windower_and_Feature_Extractor:
    ...
    def create_saving_directory(self):
        ...
        self.dire_wins_feats_silent = Path(self.data_dire_wins_and_feats/self.sub_id/'silent'/f'WIN_{int(self.win_size_sec*1000)}')
        self.dire_wins_feats_vocalized = Path(self.data_dire_wins_and_feats/self.sub_id/'vocalized'/f'WIN_{int(self.win_size_sec*1000)}')
    ...
    def main(self):
        ...
        for curr_h5_file in h5_files:
            ...
            if df_save_path.exists() == False:
                ...
                print(f"Done with: SESSION:{df_wins_feats['session_id'].unique()} - BATCH: {df_wins_feats['batch_id'].unique()} - TYPE: {curr_h5_file.parent.name }")
