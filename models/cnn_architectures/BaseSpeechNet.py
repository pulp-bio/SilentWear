...
def is_variant_valid(C, kernels, pools):
    # find the first "full" block
    full_idx = next((i for i, k in enumerate(kernels) if k[0] == "full"), None)
    if full_idx is None:
        return True

    # if you pool along channel/height before the full block, height shrinks -> invalid
    for i in range(full_idx):
        pool_c = pools[i][0]
        if int(pool_c) != 1:
            return False
    return True
...
class ChannelCollapse(nn.Module):
    def __init__(self, mode="avg"):
        ...
        self.mode = mode
    def forward(self, x):
        # x: (B, ch, H, W) -> collapse H
        if self.mode == "avg":
            return x.mean(dim=2, keepdim=True)
        return x.amax(dim=2, keepdim=True)
    
class BaseSpeechNet(nn.Module):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i, cfg in enumerate(blocks_config):
            ...
            if k_c == "full":
                # collapse current channel-height no matter what it is
                collapse_mode = cfg.get("collapse_mode", "avg")  # optional
                layers.append(ChannelCollapse(mode=collapse_mode))

                # now conv only along time (height is 1)
                conv = nn.Conv2d(in_ch, out_ch, kernel_size=(1, int(k_t)), stride=stride, padding=padding)
            else:
                conv = nn.Conv2d(in_ch, out_ch, kernel_size=(int(k_c), int(k_t)), stride=stride, padding=padding)
