...
class Model_Fine_Tuner:
    ...
    def build_and_load_weights(self):
        ...
        if self.model_to_ft_path is None:
            print("No checkpoint provided (model_to_ft_path=None) -> using RANDOMLY initialized weights.")
        ...
        if len(missing) < 20 and missing: print("  missing:", missing)
        if len(unexpected) < 20 and unexpected: print("  unexpected:", unexpected)
    ...
    def test_zero_shot_acc(self):
        ...
        self.model_master.trainer_manager.test_loader = self.model_master.trainer_manager.create_dataloader_from_df(zero_shot_test_df[self.df_col], 
                                                        batch_size = 1, 
                                                        shuffle = False, 
                                                        num_workers=0)
